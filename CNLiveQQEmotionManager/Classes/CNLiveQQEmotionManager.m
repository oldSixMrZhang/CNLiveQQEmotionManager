...
+ (NSArray<QMUIEmotion *> *)emotionsForQQ {
    if (QQEmotionArray && CNLiveEmojiLoadFailCount == 0) {
...
+ (void)asyncLoadImages:(NSArray<QMUIEmotion *> *)emotions {
    ...
    //zx 重试加载3次
    MJWeakSelf
    if (CNLiveEmojiLoadFailCount == 3) {
        return;
    } dispatch_async(dispatch_get_global_queue(0, 0), ^{
        
        BOOL isCircle = NO;
        for (QMUIEmotion *e in emotions) {
            UIImage *image = [e cnliveImage];
            if (image == nil) {
                NSLog(@"加载emoji失败");
                isCircle = YES;
                CNLiveEmojiLoadFailCount ++;
                break;
            }
            e.image = image;
        }
        if (isCircle)
        {
           [weakSelf asyncLoadImages:emotions];
        }
    });
}
